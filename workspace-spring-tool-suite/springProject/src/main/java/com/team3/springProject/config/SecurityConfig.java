...
public class SecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests((authorizeHttpRequests) -> authorizeHttpRequests
				.requestMatchers(new AntPathRequestMatcher("/user/delete")).authenticated() // 회원 탈퇴
				.requestMatchers(new AntPathRequestMatcher("/user/reservation/create")).authenticated() // 예약 생성
				.requestMatchers(new AntPathRequestMatcher("/user/reservation/delete/**")).authenticated() // 예약 삭제
				.requestMatchers(new AntPathRequestMatcher("/**")).permitAll()).csrf(csrf -> csrf
						// /h2-console/로 시작하는 모든 URL은 CSRF 검증을 하지 않겠다.
